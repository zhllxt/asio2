...
#ifndef ASIO2_PFR_DETAIL_FIELDS_COUNT_HPP
#define ASIO2_PFR_DETAIL_FIELDS_COUNT_HPP
#pragma once
...
constexpr std::size_t fields_count() noexcept {
    ...
    static_assert(
        ...
        "====================> PFR: Attempt to get fields count on a reference. This is not allowed because that could hide an issue and different library users expect different behavior in that case."
    );
    ...
    static_assert(
        ...
        "====================> PFR: Type and each field in the type must be copy constructible (or move constructible and move assignable)."
    );

    static_assert(
        !std::is_polymorphic<type>::value,
        "====================> PFR: Type must have no virtual function, because otherwise it is not aggregate initializable."
    );
    ...
    static_assert(
        ...
        "====================> PFR: Type must be aggregate initializable."
    );
#endif

// Can't use the following. See the non_std_layout.cpp test.
//#if !ASIO2_PFR_USE_CPP17
//    static_assert(
...
//#endif
    ...
    static_assert(
        ...
        "====================> PFR: Types with user specified constructors (non-aggregate initializable types) are not supported."
    );
#endif

    static_assert(
        result != 0 || std::is_empty<type>::value || std::is_fundamental<type>::value || std::is_reference<type>::value,
        "====================> PFR: If there's no other failed static asserts then something went wrong. Please report this issue to the github along with the structure you're reflecting."
    );
...
#endif // ASIO2_PFR_DETAIL_FIELDS_COUNT_HPP
