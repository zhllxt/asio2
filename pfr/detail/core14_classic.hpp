...
#ifndef ASIO2_PFR_DETAIL_CORE14_CLASSIC_HPP
#define ASIO2_PFR_DETAIL_CORE14_CLASSIC_HPP
#pragma once
...
constexpr std::size_t type_to_id_extension_apply(std::size_t ext) noexcept {
    ...
    static_assert(
        ...
        "====================> PFR: Too many extensions for a single field (something close to `int************************** p;` is in the POD type)."
    );
...
/// @cond
#define ASIO2_MAGIC_GET_REGISTER_TYPE(Type, Index)              \
...
ASIO2_MAGIC_GET_REGISTER_TYPE(unsigned char         , 1)
ASIO2_MAGIC_GET_REGISTER_TYPE(unsigned short        , 2)
ASIO2_MAGIC_GET_REGISTER_TYPE(unsigned int          , 3)
ASIO2_MAGIC_GET_REGISTER_TYPE(unsigned long         , 4)
ASIO2_MAGIC_GET_REGISTER_TYPE(unsigned long long    , 5)
ASIO2_MAGIC_GET_REGISTER_TYPE(signed char           , 6)
ASIO2_MAGIC_GET_REGISTER_TYPE(short                 , 7)
ASIO2_MAGIC_GET_REGISTER_TYPE(int                   , 8)
ASIO2_MAGIC_GET_REGISTER_TYPE(long                  , 9)
ASIO2_MAGIC_GET_REGISTER_TYPE(long long             , 10)
ASIO2_MAGIC_GET_REGISTER_TYPE(char                  , 11)
ASIO2_MAGIC_GET_REGISTER_TYPE(wchar_t               , 12)
ASIO2_MAGIC_GET_REGISTER_TYPE(char16_t              , 13)
ASIO2_MAGIC_GET_REGISTER_TYPE(char32_t              , 14)
ASIO2_MAGIC_GET_REGISTER_TYPE(float                 , 15)
ASIO2_MAGIC_GET_REGISTER_TYPE(double                , 16)
ASIO2_MAGIC_GET_REGISTER_TYPE(long double           , 17)
ASIO2_MAGIC_GET_REGISTER_TYPE(bool                  , 18)
ASIO2_MAGIC_GET_REGISTER_TYPE(void*                 , 19)
ASIO2_MAGIC_GET_REGISTER_TYPE(const void*           , 20)
ASIO2_MAGIC_GET_REGISTER_TYPE(volatile void*        , 21)
ASIO2_MAGIC_GET_REGISTER_TYPE(const volatile void*  , 22)
ASIO2_MAGIC_GET_REGISTER_TYPE(std::nullptr_t        , 23)
constexpr std::size_t tuple_begin_tag               = 24;
constexpr std::size_t tuple_end_tag                 = 25;

#undef ASIO2_MAGIC_GET_REGISTER_TYPE
...
constexpr std::size_t type_to_id(identity<Type*>) noexcept {
    ...
    static_assert(
        ...
        "====================> PFR: Pointers to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<const Type*>) noexcept {
    ...
    static_assert(
        std::is_same<const std::size_t, decltype(unptr)>::value,
        "====================> PFR: Const pointers to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<const volatile Type*>) noexcept {
    ...
    static_assert(
        std::is_same<const std::size_t, decltype(unptr)>::value,
        "====================> PFR: Const volatile pointers to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<volatile Type*>) noexcept {
    ...
    static_assert(
        std::is_same<const std::size_t, decltype(unptr)>::value,
        "====================> PFR: Volatile pointers to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<Type&>) noexcept {
    ...
    static_assert(
        std::is_same<const std::size_t, decltype(unptr)>::value,
        "====================> PFR: References to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<Type>, std::enable_if_t<std::is_empty<Type>::value>*) noexcept {
    static_assert(!std::is_empty<Type>::value, "====================> PFR: Empty classes/structures as members are not supported.");
    return 0;
}
...
constexpr std::size_t type_to_id(identity<Type>, std::enable_if_t<std::is_union<Type>::value>*) noexcept {
    static_assert(
        ...
        "====================> PFR: For safety reasons it is forbidden to reflect unions. See `Reflection of unions` section in the docs for more info."
    );
    return 0;
}
...
constexpr auto id_to_type(size_t_<Index >, if_extension<Index, native_ref_type>) noexcept {
    static_assert(!Index, "====================> PFR: References are not supported");
    return nullptr;
}
...
constexpr void* flat_type_to_array_of_type_ids(std::size_t* types, std::index_sequence<I...>) noexcept
{
    static_assert(
        N <= sizeof(T),
        "====================> PFR: Bit fields are not supported."
    );
...
constexpr auto prepare_subtuples(size_t_<V>, size_t_<I>, size_t_<SubtupleLength>) noexcept {
    static_assert(SubtupleLength == 0, "====================> PFR: Internal error while representing nested field as tuple");
...
constexpr auto prepare_subtuples(size_t_<typeid_conversions::tuple_end_tag>, size_t_<I>, size_t_<SubtupleLength>) noexcept {
    static_assert(sizeof(T) == 0, "====================> PFR: Internal error while representing nested field as tuple");
    return int{};
}

template <class T, std::size_t I, std::size_t SubtupleLength>
constexpr auto prepare_subtuples(size_t_<typeid_conversions::tuple_begin_tag>, size_t_<I>, size_t_<SubtupleLength>) noexcept {
    static_assert(SubtupleLength > 2, "====================> PFR: Internal error while representing nested field as tuple");
...
constexpr auto internal_tuple_with_same_alignment() noexcept {
    ...
    static_assert(
        ...
        "====================> PFR: Type can not be reflected without Loophole or C++17, because it's not POD"
    );
    static_assert(!std::is_reference<type>::value, "====================> PFR: Not applyable");
...
auto tie_as_tuple(T& val) noexcept {
    ...
    static_assert(
        ...
        "====================> PFR: Not possible in C++14 to represent that type without loosing information. Change type definition or enable C++17"
    );
...
struct ubiq_constructor_constexpr_copy {
    ...
    constexpr operator Type() const noexcept {
        static_assert(
            ...
            "====================> PFR: One of the fields in the type passed to `for_each_field` has non trivial destructor."
        );
        return {};
    }
};
...
void for_each_field_dispatcher(T& t, F&& f, std::index_sequence<I...>) {
    ...
    //static_assert(is_constexpr_aggregate_initializable<T, I...>::value, "====================> PFR: T must be a constexpr initializable type");
...
#endif // ASIO2_PFR_DETAIL_CORE14_CLASSIC_HPP
