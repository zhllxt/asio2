...
#ifndef ASIO2_PFR_CORE_HPP
#define ASIO2_PFR_CORE_HPP
#pragma once
...
constexpr auto get(T&, std::enable_if_t<!std::is_assignable<T, T>::value>* = nullptr) noexcept {
    static_assert(sizeof(T) && false, "====================> PFR: Calling pfr::get on non const non assignable type is allowed only in C++17");
    return 0;
}
#endif
...
constexpr auto structure_tie(T&, std::enable_if_t<!std::is_assignable<T, T>::value>* = nullptr) noexcept {
    static_assert(sizeof(T) && false, "====================> PFR: Calling pfr::structure_tie on non const non assignable type is allowed only in C++17");
    return 0;
}
#endif
...
constexpr auto structure_tie(T&&, std::enable_if_t< std::is_rvalue_reference<T&&>::value>* = 0) noexcept {
    static_assert(sizeof(T) && false, "====================> PFR: Calling pfr::structure_tie on rvalue references is forbidden");
    return 0;
}
