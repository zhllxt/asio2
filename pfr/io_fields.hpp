...
#ifndef ASIO2_PFR_IO_FIELDS_HPP
#define ASIO2_PFR_IO_FIELDS_HPP
#pragma once
...
std::basic_ostream<Char, Traits>& operator<<(std::basic_ostream<Char, Traits>& out, io_fields_impl<const T&>&& x) {
    ...
    out << '{';
#if ASIO2_PFR_USE_CPP17 || ASIO2_PFR_USE_LOOPHOLE
...
std::basic_istream<Char, Traits>& operator>>(std::basic_istream<Char, Traits>& in, io_fields_impl<const T&>&& ) {
    static_assert(sizeof(T) && false, "====================> PFR: Atetmpt to use istream operator on a pfr::io_fields wrapped type T with const qualifier.");
    return in;
}

template <class Char, class Traits, class T>
std::basic_istream<Char, Traits>& operator>>(std::basic_istream<Char, Traits>& in, io_fields_impl<T>&& ) {
    static_assert(sizeof(T) && false, "====================> PFR: Atetmpt to use istream operator on a pfr::io_fields wrapped temporary of type T.");
    return in;
}
