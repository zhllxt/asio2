...
#ifndef __ASIO2_HTTP_REQUEST_PARSER_HPP__
#define __ASIO2_HTTP_REQUEST_PARSER_HPP__
...
namespace asio2
{
	...
	protected:
		...
		http::http_parser_settings & get_http_parser_settings()
		{
			static http::http_parser_settings settings =
			{
				...
				{
					return static_cast<http_request_parser *>(parser->data)->on_message_begin(user_data);
				},
				[](http::http_parser * parser, const char *at, size_t length, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_url(at, length, user_data);
				},
				[](http::http_parser * parser, const char *at, size_t length, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_status(at, length, user_data);
				},
				[](http::http_parser * parser, const char *at, size_t length, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_header_field(at, length, user_data);
				},
				[](http::http_parser * parser, const char *at, size_t length, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_header_value(at, length, user_data);
				},
				[](http::http_parser * parser, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_headers_complete(user_data);
				},
				[](http::http_parser * parser, const char *at, size_t length, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_body(at, length, user_data);
				},
				[](http::http_parser * parser, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_message_complete(user_data);
				},
				[](http::http_parser * parser, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_chunk_header(user_data);
				},
				[](http::http_parser * parser, void * user_data) -> int
				{
					return static_cast<http_request_parser *>(parser->data)->on_chunk_complete(user_data);
				}
			};
			return settings;
		}
	...
	protected:
		...
		{
			static_cast<http_request*>(user_data)->method(m_parser.method);
			return ((m_parser.method <= 32) ? 0 : 1); // see http_parser.h,HTTP_METHOD_MAP
		}
		int on_url(const char *at, size_t length, void * user_data)
		{
			static_cast<http_request*>(user_data)->uri(at, length);
			if (!url_decode(static_cast<http_request*>(user_data)->m_uri))
				return 1;
			return 0;
		}
		...
		int on_header_field(const char *at, size_t length, void * user_data)
		{
			...
			case VALUE:
				...
				static_cast<http_request*>(user_data)->add_header(header_field, header_value);
				header_field.assign(at, length);
				break;
			case FIELD:
		...
		{
			static_cast<http_request*>(user_data)->http_version(m_parser.http_major, m_parser.http_minor);
			...
			std::string extension = get_extension_for_mime_type(static_cast<http_request*>(user_data)->get_header_value("content-type"));

			/* Choose an output function based on the content encoding */
			std::string content_encoding(static_cast<http_request*>(user_data)->get_header_value("content-encoding"));
...
#endif // !__ASIO2_HTTP_REQUEST_PARSER_HPP__
