...
namespace asio2
{
	...
	class tcp_auto_connection_impl : public tcp_connection_impl<_pool_t>
	{
	public:
	...
	protected:
			...
			if (this->is_start())
			{
				if (m_recv_is_header)
				{
					...
					boost::asio::async_read(*this->m_socket_ptr,
						...
						this->m_evt_recv_strand_ptr->wrap(std::bind(&tcp_auto_connection_impl::_handle_recv, std::static_pointer_cast<tcp_auto_connection_impl>(this->shared_from_this()),
		...
		virtual void _post_send(std::shared_ptr<uint8_t> send_buf_ptr, std::size_t len) override
		{
			...
			tcp_connection_impl<_pool_t>::_post_send(send_buf_ptr, len);
		}
