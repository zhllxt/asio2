	...
	protected:
		...
		virtual bool _start_send()
		{
			try
			{
				...
				boost::asio::ip::icmp::resolver resolver(*m_evt_recv_ioservice_ptr);
				...
				m_socket_ptr = std::make_shared<boost::asio::ip::icmp::socket>(*m_evt_recv_ioservice_ptr, boost::asio::ip::icmp::v4());
			...
			if (is_start())
			{
				...
				try
				{
					...
					m_socket_ptr->async_receive(
						...
						m_evt_recv_strand_ptr->wrap(std::bind(&icmp_sender_impl::_handle_recv, 
		...
		virtual void _fire_recv(boost::asio::ip::udp::endpoint sender_endpoint, std::shared_ptr<uint8_t> recv_buf_ptr, std::size_t bytes_recvd)
		{
			if (is_start() && m_listener_mgr_ptr->is_recv_listener_exist())
			{
				if (m_async_notify)
				{
					...
					m_msg_recv_strand_ptr->post(std::bind(&icmp_sender_impl::_do_fire_recv,
						std::dynamic_pointer_cast<icmp_sender_impl>(shared_from_this()),
		...
		virtual void _fire_send(boost::asio::ip::udp::endpoint recver_endpoint, std::shared_ptr<uint8_t> send_buf_ptr, std::size_t bytes_sent, int error)
		{
			if (is_start() && m_listener_mgr_ptr->is_send_listener_exist())
			{
				if (m_async_notify)
				{
					m_msg_send_strand_ptr->post(std::bind(&icmp_sender_impl::_do_fire_send,
						std::dynamic_pointer_cast<icmp_sender_impl>(shared_from_this()),
