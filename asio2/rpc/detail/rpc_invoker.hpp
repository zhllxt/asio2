...
namespace asio2::detail
{
	...
	protected:
		...
		{
			//std::unique_lock<std::shared_mutex> guard(this->mutex_);
			this->invokers_[std::move(name)] = std::bind(&self::template _proxy<F>,
				this, std::move(f),
			...
			inline _argc_proxy(F& f, std::shared_ptr<CallerT>& caller, rpc_serializer& sr, rpc_deserializer& dr)
		{
			...
			if constexpr (std::is_same_v<std::shared_ptr<CallerT>, arg0_type>)
			{
				...
				dr >> tp;
				_invoke<fun_ret_type>(f, sr, dr, std::tuple_cat(
					std::tuple<std::shared_ptr<CallerT>&>(caller), tp));
			}
			else
			{
				fun_args_tuple tp;
			...
			inline _argc_proxy(F& f, C* c, std::shared_ptr<CallerT>& caller, rpc_serializer& sr, rpc_deserializer& dr)
		{
			...
			if constexpr (std::is_same_v<std::shared_ptr<CallerT>, arg0_type>)
			{
				auto tp = _body_args_tuple((fun_args_tuple*)0);
				dr >> tp;
				_invoke<fun_ret_type>(f, c, sr, dr, std::tuple_cat(
					std::tuple<std::shared_ptr<CallerT>&>(caller), tp));
			}
			else
			{
