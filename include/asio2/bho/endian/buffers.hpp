...
#   define BHO_ENDIAN_DEFAULT_CONSTRUCT {}          // C++03
# else
#   define BHO_ENDIAN_DEFAULT_CONSTRUCT = default;  // C++0x
# endif

// g++ pre-4.6 does not support unrestricted unions, but we have no Config macro for that
# if (defined(BHO_NO_CXX11_DEFAULTED_FUNCTIONS) || BHO_WORKAROUND(BHO_GCC, < 40600)) && defined(BHO_ENDIAN_FORCE_PODNESS)
#   define BHO_ENDIAN_NO_CTORS
# endif
...
#   endif
  ...
  template <BHO_SCOPED_ENUM(order) Order, class T, std::size_t n_bits,
    BHO_SCOPED_ENUM(align) A = align::no>
      class endian_buffer;
  ...
  // Stream inserter
  template <class charT, class traits, BHO_SCOPED_ENUM(order) Order, class T,
    std::size_t n_bits, BHO_SCOPED_ENUM(align) A>
...
public:
    ...
    BHO_STATIC_ASSERT( (n_bits/8)*8 == n_bits );
...
public:
    ...
    endian_buffer() BHO_ENDIAN_DEFAULT_CONSTRUCT
...
private:
    ...
    BHO_STATIC_ASSERT( (n_bits/8)*8 == n_bits );
    BHO_STATIC_ASSERT( sizeof(T) == n_bits/8 );
