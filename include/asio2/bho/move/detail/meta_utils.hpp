...
{
   T & v_;
   BHO_MOVE_FORCEINLINE addr_impl_ref( T & v ): v_( v ) {}
   BHO_MOVE_FORCEINLINE operator T& () const { return v_; }
...
{
   BHO_MOVE_FORCEINLINE static T * f( T & v, long )
   {
      return reinterpret_cast<T*>(
         &const_cast<char&>(reinterpret_cast<const volatile char &>(v)));
   }

   BHO_MOVE_FORCEINLINE static T * f( T * v, int )
   {  return v;  }
};

template<class T>
BHO_MOVE_FORCEINLINE T * addressof( T & v )
{
   return ::bho::move_detail::addressof_impl<T>::f
      ( ::bho::move_detail::addr_impl_ref<T>( v ), 0 );
}
