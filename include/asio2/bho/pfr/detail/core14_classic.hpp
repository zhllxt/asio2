...
#ifndef BHO_PFR_DETAIL_CORE14_CLASSIC_HPP
...
constexpr std::size_t type_to_id_extension_apply(std::size_t ext) noexcept {
    ...
    static_assert(
        ...
        "====================> BHO.PFR: Too many extensions for a single field (something close to `int************************** p;` is in the POD type)."
    );
...
/// @cond
#define BHO_MAGIC_GET_REGISTER_TYPE(Type, Index)              \
...
BHO_MAGIC_GET_REGISTER_TYPE(unsigned char         , 1)
BHO_MAGIC_GET_REGISTER_TYPE(unsigned short        , 2)
BHO_MAGIC_GET_REGISTER_TYPE(unsigned int          , 3)
BHO_MAGIC_GET_REGISTER_TYPE(unsigned long         , 4)
BHO_MAGIC_GET_REGISTER_TYPE(unsigned long long    , 5)
BHO_MAGIC_GET_REGISTER_TYPE(signed char           , 6)
BHO_MAGIC_GET_REGISTER_TYPE(short                 , 7)
BHO_MAGIC_GET_REGISTER_TYPE(int                   , 8)
BHO_MAGIC_GET_REGISTER_TYPE(long                  , 9)
BHO_MAGIC_GET_REGISTER_TYPE(long long             , 10)
BHO_MAGIC_GET_REGISTER_TYPE(char                  , 11)
BHO_MAGIC_GET_REGISTER_TYPE(wchar_t               , 12)
BHO_MAGIC_GET_REGISTER_TYPE(char16_t              , 13)
BHO_MAGIC_GET_REGISTER_TYPE(char32_t              , 14)
BHO_MAGIC_GET_REGISTER_TYPE(float                 , 15)
BHO_MAGIC_GET_REGISTER_TYPE(double                , 16)
BHO_MAGIC_GET_REGISTER_TYPE(long double           , 17)
BHO_MAGIC_GET_REGISTER_TYPE(bool                  , 18)
BHO_MAGIC_GET_REGISTER_TYPE(void*                 , 19)
BHO_MAGIC_GET_REGISTER_TYPE(const void*           , 20)
BHO_MAGIC_GET_REGISTER_TYPE(volatile void*        , 21)
BHO_MAGIC_GET_REGISTER_TYPE(const volatile void*  , 22)
BHO_MAGIC_GET_REGISTER_TYPE(std::nullptr_t        , 23)
constexpr std::size_t tuple_begin_tag               = 24;
constexpr std::size_t tuple_end_tag                 = 25;

#undef BHO_MAGIC_GET_REGISTER_TYPE
...
constexpr std::size_t type_to_id(identity<Type*>) noexcept {
    ...
    static_assert(
        ...
        "====================> BHO.PFR: Pointers to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<const Type*>) noexcept {
    ...
    static_assert(
        std::is_same<const std::size_t, decltype(unptr)>::value,
        "====================> BHO.PFR: Const pointers to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<const volatile Type*>) noexcept {
    ...
    static_assert(
        std::is_same<const std::size_t, decltype(unptr)>::value,
        "====================> BHO.PFR: Const volatile pointers to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<volatile Type*>) noexcept {
    ...
    static_assert(
        std::is_same<const std::size_t, decltype(unptr)>::value,
        "====================> BHO.PFR: Volatile pointers to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<Type&>) noexcept {
    ...
    static_assert(
        std::is_same<const std::size_t, decltype(unptr)>::value,
        "====================> BHO.PFR: References to user defined types are not supported."
    );
...
constexpr std::size_t type_to_id(identity<Type>, std::enable_if_t<std::is_empty<Type>::value>*) noexcept {
    static_assert(!std::is_empty<Type>::value, "====================> BHO.PFR: Empty classes/structures as members are not supported.");
    return 0;
}
...
constexpr auto id_to_type(size_t_<Index >, if_extension<Index, native_ref_type>) noexcept {
    static_assert(!Index, "====================> BHO.PFR: References are not supported");
    return nullptr;
}
...
constexpr void* flat_type_to_array_of_type_ids(std::size_t* types, std::index_sequence<I...>) noexcept
{
    static_assert(
        N <= sizeof(T),
        "====================> BHO.PFR: Bit fields are not supported."
    );
...
constexpr auto prepare_subtuples(size_t_<V>, size_t_<I>, size_t_<SubtupleLength>) noexcept {
    static_assert(SubtupleLength == 0, "====================> BHO.PFR: Internal error while representing nested field as tuple");
...
constexpr auto prepare_subtuples(size_t_<typeid_conversions::tuple_end_tag>, size_t_<I>, size_t_<SubtupleLength>) noexcept {
    static_assert(sizeof(T) == 0, "====================> BHO.PFR: Internal error while representing nested field as tuple");
    return int{};
}

template <class T, std::size_t I, std::size_t SubtupleLength>
constexpr auto prepare_subtuples(size_t_<typeid_conversions::tuple_begin_tag>, size_t_<I>, size_t_<SubtupleLength>) noexcept {
    static_assert(SubtupleLength > 2, "====================> BHO.PFR: Internal error while representing nested field as tuple");
...
constexpr auto internal_tuple_with_same_alignment() noexcept {
    ...
    static_assert(
        ...
        "====================> BHO.PFR: Type can not be reflected without Loophole or C++17, because it's not POD"
    );
    static_assert(!std::is_reference<type>::value, "====================> BHO.PFR: Not applyable");
...
auto tie_as_flat_tuple(T& lvalue) noexcept {
    ...
    return bho::pfr::detail::make_flat_tuple_of_references(lvalue, getter, size_t_<0>{}, size_t_<tuple_type::size_v>{});
}
...
auto tie_as_tuple(T& val) noexcept {
    ...
    static_assert(
        bho::pfr::detail::is_flat_refelectable<T>( detail::make_index_sequence<bho::pfr::detail::fields_count<T>()>{} ),
        "====================> BHO.PFR: Not possible in C++14 to represent that type without loosing information. Change type definition or enable C++17"
    );
    return bho::pfr::detail::tie_as_flat_tuple(val);
}
...
struct ubiq_constructor_constexpr_copy {
    ...
    constexpr operator Type() const noexcept {
        static_assert(
            ...
            "====================> BHO.PFR: One of the fields in the type passed to `for_each_field` has non trivial destructor."
        );
        return {};
    }
};
...
struct next_step {
    ...
    operator Field() const {
         bho::pfr::detail::for_each_field_in_depth(
             t,
             std::forward<F>(f),
             IndexSeq{},
             identity<Fields>{}...,
...
void for_each_field_in_depth(T& lvalue, F&& f, std::index_sequence<>, identity<Fields>...) {
    ...
    std::forward<F>(f)(
        bho::pfr::detail::make_flat_tuple_of_references(lvalue, getter, size_t_<0>{}, size_t_<sizeof...(Fields)>{})
    );
}
...
void for_each_field_dispatcher_1(T& t, F&& f, std::index_sequence<I...>, std::true_type /*is_flat_refelectable*/) {
    std::forward<F>(f)(
        bho::pfr::detail::tie_as_flat_tuple(t)
    );
}
...
#endif // BHO_PFR_DETAIL_CORE14_CLASSIC_HPP
