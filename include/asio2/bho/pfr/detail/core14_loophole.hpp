...
struct fn_def_lref {
    friend auto loophole(tag<T,N>) {
        ...
        return static_cast< std::conditional_t<std::is_move_constructible<no_extents_t>::value, no_extents_t&&, no_extents_t&> >(
            bho::pfr::detail::unsafe_declval<no_extents_t&>()
        );
    }
};
template <class T, class U, std::size_t N, bool B>
struct fn_def_rref {
    friend auto loophole(tag<T,N>) { return std::move(bho::pfr::detail::unsafe_declval< std::remove_all_extents_t<U>& >()); }
};
...
auto tie_as_tuple_loophole_impl(T& lvalue) noexcept {
    ...
    return bho::pfr::detail::make_flat_tuple_of_references(
        lvalue,
...
auto tie_as_tuple(T& val) noexcept {
    ...
    return bho::pfr::detail::tie_as_tuple_loophole_impl(
        val
    );
}
...
void for_each_field_dispatcher(T& t, F&& f, std::index_sequence<I...>) {
    ...
    std::forward<F>(f)(
        bho::pfr::detail::tie_as_tuple_loophole_impl(t)
    );
}
...
#endif // BHO_PFR_DETAIL_CORE14_LOOPHOLE_HPP
