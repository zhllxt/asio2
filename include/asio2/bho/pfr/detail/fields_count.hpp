...
#ifndef BHO_PFR_DETAIL_FIELDS_COUNT_HPP
...
constexpr bool static_assert_non_inherited() noexcept {
    static_assert(
            ...
            "====================> BHO.PFR: BHO.PFR: Inherited types are not supported."
    );
    return true;
}
...
constexpr std::size_t fields_count() noexcept {
    ...
    static_assert(
        ...
        "====================> BHO.PFR: Attempt to get fields count on a reference. This is not allowed because that could hide an issue and different library users expect different behavior in that case."
    );

#if !BHO_PFR_HAS_GUARANTEED_COPY_ELISION
    static_assert(
        ...
        "====================> BHO.PFR: Type and each field in the type must be copy constructible (or move constructible and move assignable)."
    );
#endif  // #if !BHO_PFR_HAS_GUARANTEED_COPY_ELISION

    static_assert(
        !std::is_polymorphic<type>::value,
        "====================> BHO.PFR: Type must have no virtual function, because otherwise it is not aggregate initializable."
    );
    ...
    static_assert(
        ...
        "====================> BHO.PFR: Type must be aggregate initializable."
    );
#endif
...
//#endif
    ...
    static_assert(
        ...
        "====================> BHO.PFR: Types with user specified constructors (non-aggregate initializable types) are not supported."
    );
#endif

    static_assert(
        result != 0 || std::is_empty<type>::value || std::is_fundamental<type>::value || std::is_reference<type>::value,
        "====================> BHO.PFR: If there's no other failed static asserts then something went wrong. Please report this issue to the github along with the structure you're reflecting."
    );
...
#endif // BHO_PFR_DETAIL_FIELDS_COUNT_HPP
