...
///         return os << bho::pfr::io_fields(x);  // Equivalent to: os << "{ " << x.i << " ," <<  x.s << " }"
///     }
///
///     std::istream& operator>>(std::istream& is, my_struct& x) {
///         return is >> bho::pfr::io_fields(x);  // Equivalent to: is >> "{ " >> x.i >> " ," >>  x.s >> " }"
///     }
/// \endcode
///
...
std::basic_ostream<Char, Traits>& operator<<(std::basic_ostream<Char, Traits>& out, io_fields_impl<const T&>&& x) {
    ...
    constexpr std::size_t fields_count_val = bho::pfr::detail::fields_count<T>();
    out << '{';
    ...
    ::bho::pfr::detail::for_each_field_dispatcher(
        ...
        [&out](const auto& val) {
            ...
            constexpr std::size_t fields_count_val_lambda = bho::pfr::detail::fields_count<T>();
...
std::basic_istream<Char, Traits>& operator>>(std::basic_istream<Char, Traits>& in, io_fields_impl<const T&>&& ) {
    static_assert(sizeof(T) && false, "====================> BHO.PFR: Attempt to use istream operator on a bho::pfr::io_fields wrapped type T with const qualifier.");
    return in;
}

template <class Char, class Traits, class T>
std::basic_istream<Char, Traits>& operator>>(std::basic_istream<Char, Traits>& in, io_fields_impl<T>&& ) {
    static_assert(sizeof(T) && false, "====================> BHO.PFR: Attempt to use istream operator on a bho::pfr::io_fields wrapped temporary of type T.");
    return in;
}
...
/// \endcode
///
/// Input and output streaming operators for `bho::pfr::io_fields` are symmetric, meaning that you get the original value by streaming it and
