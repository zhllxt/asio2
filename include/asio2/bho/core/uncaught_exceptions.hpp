...
#define BHO_CORE_HAS_UNCAUGHT_EXCEPTIONS
#endif
#else
#define BHO_CORE_HAS_UNCAUGHT_EXCEPTIONS
...
#if !defined(BHO_CORE_HAS_UNCAUGHT_EXCEPTIONS)

// cxxabi.h availability macro
#if defined(__has_include) && (!defined(BHO_GCC) || (__GNUC__ >= 5))
...
#include <cxxabi.h>
#include <cstring>
#define BHO_CORE_HAS_CXA_GET_GLOBALS
...
#else
extern "C" __cxa_eh_globals* __cxa_get_globals() BHO_NOEXCEPT_OR_NOTHROW __attribute__((__const__));
#endif
} // namespace __cxxabiv1
#endif
#endif
#endif // defined(BHO_CORE_HAS_CXXABI_H)

#if defined(_MSC_VER) && _MSC_VER >= 1400
#include <cstring>
#define BHO_CORE_HAS_GETPTD
namespace bho {
namespace core {
namespace detail {
...
#endif // !defined(BHO_CORE_HAS_UNCAUGHT_EXCEPTIONS)

#if !defined(BHO_CORE_HAS_UNCAUGHT_EXCEPTIONS) && !defined(BHO_CORE_HAS_CXA_GET_GLOBALS) && !defined(BHO_CORE_HAS_GETPTD)
...
#define BHO_CORE_UNCAUGHT_EXCEPTIONS_EMULATED
#endif
...
inline unsigned int uncaught_exceptions() BHO_NOEXCEPT
{
#if defined(BHO_CORE_HAS_UNCAUGHT_EXCEPTIONS)
    // C++17 implementation
    return static_cast< unsigned int >(std::uncaught_exceptions());
#elif defined(BHO_CORE_HAS_CXA_GET_GLOBALS)
    ...
    return count;
#elif defined(BHO_CORE_HAS_GETPTD)
    // MSVC specific. Tested on {MSVC2005SP1,MSVC2008SP1,MSVC2010SP1,MSVC2012}x{x32,x64}.
    unsigned int count;
    std::memcpy(&count, static_cast< const unsigned char* >(bho::core::detail::_getptd()) + (sizeof(void*) == 8u ? 0x100 : 0x90), sizeof(count)); // _tiddata::_ProcessingThrow, x32 offset - 0x90, x64 - 0x100
    return count;
#else
...
#undef BHO_CORE_HAS_CXA_GET_GLOBALS
#undef BHO_CORE_HAS_UNCAUGHT_EXCEPTIONS
#undef BHO_CORE_HAS_GETPTD

#endif // BHO_CORE_UNCAUGHT_EXCEPTIONS_HPP_INCLUDED_
