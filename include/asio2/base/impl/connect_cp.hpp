	...
	protected:
		...
		_start_connect(std::shared_ptr<derived_t> this_ptr, std::shared_ptr<ecs_t<C>> ecs, DeferEvent chain)
		{
			...
			try
			{
				...
				derive._post_resolve(this_ptr, ecs, std::move(chain));
			}
			catch (system_error const& e)
			{
				set_last_error(e);

				derive._handle_connect(e.code(), std::move(this_ptr), std::move(ecs), std::move(chain));
			}
			...
			std::shared_ptr<derived_t> this_ptr, std::shared_ptr<ecs_t<C>> ecs, DeferEvent chain)
		{
			...
			try
			{
				...
				{
					if (ec)
						return derive._handle_proxy(ec, this_ptr, ecs, std::move(chain));
					...
					if constexpr (C::has_socks5())
						derive._socks5_start(this_ptr, ecs, std::move(chain));
					else
						derive._handle_proxy(ec, this_ptr, ecs, std::move(chain));
				}
				else
				{
					derive._handle_proxy(ec, this_ptr, ecs, std::move(chain));
